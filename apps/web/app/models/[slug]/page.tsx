...
export default function ModelDetailPage({ params }: { params: { slug: string } }) {
  ...
  if (!modelQuery.data) {
    return <Card>Model bilgileri yükleniyor...</Card>;
  }
    ...
    <div className="grid gap-4">
      <Card>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold">{model.brand} {model.model}</h1>
            {model.generation && <p className="text-sm text-slate-400">{model.generation}</p>}
            {model.description && <p className="mt-2 text-sm text-slate-300">{model.description}</p>}
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <h2 className="text-lg font-semibold">Son açılan konular</h2>
          <div className="mt-3 grid gap-2">
            {threads.slice(0, 8).map((thread) => (
              <Link key={thread.id} href={`/thread/${thread.slug}`} className="rounded border border-slate-700 p-3 text-sm hover:border-slate-500">
                ...
                <p className="text-xs text-slate-400">👁 {thread.view_count} · 💬 {thread.reply_count}</p>
              </Link>
            ))}
          </div>
        </Card>
        ...
        <Card>
          <h2 className="text-lg font-semibold">Kronik sorun kartları</h2>
          <div className="mt-3 grid gap-2">
            {issues.slice(0, 8).map((issue) => (
              <div key={issue.id} className="rounded border border-slate-700 p-3">
                <div className="flex items-center justify-between">
                  <p className="font-semibold">{issue.title}</p>
                  <Badge variant={issue.severity >= 4 ? 'danger' : 'outline'}>Seviye {issue.severity}</Badge>
                </div>
                {issue.body && <p className="mt-1 text-sm text-slate-300">{issue.body}</p>}
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card>
          <h2 className="text-lg font-semibold">Bakım maliyeti içgörüleri</h2>
          <div className="mt-3 grid gap-2">
            {maintenanceLogs.slice(0, 8).map((row) => (
              <div key={row.id} className="rounded border border-slate-700 p-3 text-sm">
                ...
                <p className="text-slate-400">{new Date(row.done_date).toLocaleDateString('tr-TR')} · {row.done_km ?? '-'} km · {row.username}</p>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card>
          <h2 className="text-lg font-semibold">Model shortlist</h2>
          <p className="mt-2 text-sm text-slate-300">Karşılaştırma için modele göre shortlist oluşturma alanı (E92 vs F30 gibi) v1.</p>
