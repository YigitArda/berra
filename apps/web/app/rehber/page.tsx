...
export default function RehberPage() {
  ...
  return (
    <div className="grid gap-4">
      <Card>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">İlk Araba Rehberi</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">6 adımlı seçim sihirbazı.</p>
        <div className="mt-3 h-2 rounded bg-slate-200 dark:bg-slate-800">
          <div className="h-2 rounded bg-primary" style={{ width: `${progress}%` }} />
        </div>
        <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">%{progress} tamamlandı</p>
      </Card>

      <Card>
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Adım {step + 1}: {steps[step]}</h2>

        {step === 0 && (
          <div className="mt-3 grid gap-3">
            <p className="text-sm text-slate-600 dark:text-slate-300">Bütçeni belirle (₺200.000 - ₺2.000.000)</p>
            <input type="range" min={200000} max={2000000} step={10000} value={budget} onChange={(e) => setBudget(Number(e.target.value))} />
            <p className="font-semibold text-slate-900 dark:text-slate-100">Seçilen bütçe: ₺{budget.toLocaleString('tr-TR')}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">Öneri: Toplam sahip olma maliyetini (yakıt + bakım + sigorta) aylık planla.</p>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            <div className="rounded border border-slate-300 p-3 dark:border-slate-700">
              <p className="font-semibold text-slate-900 dark:text-slate-100">Sıfır Araç</p>
            ...
            <div className="rounded border border-slate-300 p-3 dark:border-slate-700">
              <p className="font-semibold text-slate-900 dark:text-slate-100">İkinci El</p>
              <ul className="mt-2 list-disc pl-5 text-sm text-slate-600 dark:text-slate-300">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {(['Benzin', 'Dizel', 'LPG', 'Elektrik'] as const).map((item) => (
              <button
                key={item}
                type="button"
                className={`rounded border p-3 text-left ${fuel === item ? 'border-primary bg-primary/10' : 'border-slate-300 dark:border-slate-700'}`}
                onClick={() => setFuel(item)}
              >
                <p className="font-semibold text-slate-900 dark:text-slate-100">{item}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">Kullanım senaryosuna göre seç.</p>
              </button>
            ))}
          </div>
        )}
        ...
        {step === 3 && (
          <div className="mt-3 grid gap-2">
            <p className="text-sm text-slate-600 dark:text-slate-300">Bütçe ve yakıt tercihine göre örnek modeller:</p>
            <div className="flex flex-wrap gap-2">
              {suggestedModels.map((model) => (
                <span key={model} className="rounded-full border border-slate-300 px-3 py-1 text-sm text-slate-700 dark:border-slate-700 dark:text-slate-300">{model}</span>
              ))}
            </div>
          </div>
        )}
