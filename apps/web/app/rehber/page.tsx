      ...
      <Card>
        ...
        {step === 0 && (
          <div className="mt-3 grid gap-3">
            <p className="text-sm text-slate-600 dark:text-slate-300">
              Bütçeni belirle (200.000 TL - 2.000.000 TL)
            </p>
            <input
              type="range"
              min={200000}
            ...
            <p className="font-semibold text-slate-900 dark:text-slate-100">
              Seçilen bütçe: {budget.toLocaleString('tr-TR')} TL
            </p>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Öneri: Toplam sahip olma maliyetini yakıt, bakım ve sigorta ile birlikte aylık planla.
            </p>
          </div>
        )}
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {(['Benzin', 'Dizel', 'LPG', 'Elektrik'] as const).map((item) => (
              <button
                key={item}
                type="button"
                className={`rounded-lg border p-3 text-left transition ${fuel === item ? 'border-primary bg-primary/10' : 'border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-900'}`}
                onClick={() => setFuel(item)}
              >
                <p className="font-semibold text-slate-900 dark:text-slate-100">{item}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Kullanım senaryosuna göre seç.
                </p>
              </button>
            ))}
          </div>
        ...
        {step === 3 && (
          <div className="mt-3 grid gap-2">
            <p className="text-sm text-slate-600 dark:text-slate-300">
              Bütçe ve yakıt tercihine göre örnek modeller:
            </p>
        ...
        {step === 5 && (
          <ul className="mt-3 list-disc pl-5 text-sm text-slate-600 dark:text-slate-300">
            <li>Zorunlu trafik ve kasko sigortasını tamamla</li>
